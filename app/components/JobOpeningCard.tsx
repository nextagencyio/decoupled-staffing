...
interface JobOpeningCardProps {
  item: DrupalJobOpening
}

export default function JobOpeningCard({ item }: JobOpeningCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="flex-1 p-6 flex flex-col justify-center">
        {(item as any).location && (
          <p className="text-sm text-primary-600 font-medium mb-1">{(item as any).location}</p>
        )}
