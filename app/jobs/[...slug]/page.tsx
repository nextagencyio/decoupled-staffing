...
import { GET_JOB_OPENING_BY_PATH } from '@/lib/queries'
...
interface JobOpeningByPathData {
  route: {
    entity: DrupalJobOpening
  } | null
}

async function getJobOpening(path: string): Promise<DrupalJobOpening | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<JobOpeningByPathData>({
      query: GET_JOB_OPENING_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/jobs/${slug.join('/')}`
  const item = await getJobOpening(path)

  if (!item) {
    return { title: 'Job Opening Not Found | Staffing' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).companyName && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).companyName}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).employmentType && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).employmentType}</dd>
                    </div>
                  )}
                  {(item as any).salaryRange && (
                    <div>
                      <dt className="text-sm text-gray-500">Salary Range</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).salaryRange}</dd>
                    </div>
                  )}
                  {(item as any).jobIndustry && (
                    <div>
                      <dt className="text-sm text-gray-500">Industry</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).jobIndustry}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
